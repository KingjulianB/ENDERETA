...
| 4 | Whether OpenAerialMap's Luanda imagery (0.5m/pixel, CC-BY 4.0) can feed the SpaceNet building-detection model | Never tried — resolution is right at SpaceNet's training threshold (~30-50cm), genuinely might work now that the pilot is Luanda, unlike Huambo where no usable imagery existed at all | `discrepancies.md` § Sovereign building/road detection model |
